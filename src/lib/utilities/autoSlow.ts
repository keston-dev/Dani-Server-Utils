...
import { AutoSlowModel } from "models/AutoSlow.ts";
...
export async function getOrCreateAutoSlow(
  cache: CachedAutoSlow,
  channelId: string,
): Promise<AutoSlowManager | null> {
  const cached = cache.getAutoSlow(channelId);
  if (cached) return cached;

  const config = await AutoSlowModel.findOne({ channelId });
  if (!config) return null;

  const manager = new AutoSlowManager(
    config.min,
    config.max,
    config.targetMsgsPerSec,
    config.minChangeRate,
    config.minChange,
    config.enabled,
  );
  cache.addAutoSlow(channelId, manager);
  return manager;
}
