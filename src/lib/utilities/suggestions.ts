...
import { SuggestionConfigModel, SuggestionModel } from "models/Suggestion.ts";

import { clientConfig } from "../config/ClientConfig.ts";
import DefaultClientUtilities from "./miscUtilities.ts";
import { ISuggestionConfig } from "types/mongodb.ts";
...
export class SuggestionUtility {
  ...
  async approve(interaction: MessageContextMenuCommandInteraction) {
  ...
  async deny(interaction: ModalSubmitInteraction, reason: string) {
    ...
    suggestionConfig.existingSubmissions = suggestionConfig.existingSubmissions.filter(
      (id) => !id.equals(suggestion._id.toString()),
    );
  ...
  generateApprovalEmbed(content: string) {
    return {
