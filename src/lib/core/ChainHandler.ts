...
export class ChainHandler {
  ...
  private async warn(client: DsuClient, channel: TextChannel): Promise<void> {
    const warning = await channel
      .send({
        embeds: [
          client.utilities.misc.generateEmbed("error", {
  ...
  public async handleMessage(client: DsuClient, message: Message): Promise<boolean> {
    ...
    if (isChain && state.chainStreak >= CONFIG.deleteThreshold) {
      ...
      if (state.chainStreak === CONFIG.warnThreshold)
        await this.warn(client, message.channel as TextChannel);
